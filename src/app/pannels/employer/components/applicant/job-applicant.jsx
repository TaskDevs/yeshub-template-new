...
import InviteeInterviewModal from "./invitee-interview-modal";
...
export default function JobApplicant() {
  ...
  const tabs = ["Job Details", "Applicants", "Invites"];
  ...
  const [openMenuTwo, setOpenMenuTwo] = useState(null);
  const [interviewStatus, setInterviewStatus] = useState(null);
  const [isInterviewOpen, setIsInterviewOpen] = useState(false);
  const [isInviteeInterviewOpen, setIsInviteeInterviewOpen] = useState(false);
  ...
  const toggleMenuTwo = (index) => {
    setOpenMenuTwo(openMenuTwo === index ? null : index);
  };
  ...
  const handleMenuTwoAction = (action, item) => {
    switch (action) {
      case "view":
        handleViewInvitee(item);
        break;
      case "hired":
        handleHireInvitee(item);
        break;
      case "rejected":
        handleRejectInvitee(item, "rejected");
        break;
      case "delete":
        handleDeleteInvitee(item);
        break;
      default:
        console.warn("Unknown action:", action);
    }
  ...
  const handleViewInvitee = (item) => {
    navigate(
      `/dashboard-client/candidate-details/${item.candidate_id}/invitee`
    );
    console.log(item);
  };
  ...
  const handleInviteeInterviewSet = (item) => {
    console.log(item);
    let newData = {
    ...
    setCandidateData(newData);
    setIsInviteeInterviewOpen(true);
  };
  ...
  // processGetInviteeInterviewInfo
  const handleViewInviteeInterviewData = (item) => {
    setInterviewStatus("view");
    processGetInviteeInterviewInfo(item.candidate_id);
    setIsInterviewOpen(true);
  };
  ...
  const handleHireInvitee = (item) => {
    console.log(item);
    let newData = {
      employer_id: userId,
      proposal_id: item.invitation_id,
      freelance_id: item.candidate_id,
      job_id: 0,
    };
    // console.log(newData);
    setCandidateSalaryData(newData);
  ...
  const handleRejectInvitee = async (data, status) => {
    console.log(data);
    let newData = {
      status: status,
    };
    let response = await processChangeInviteeStatus(
      newData,
      data.invitation_id
    );
    if (response == true) {
  ...
  //
  const handleHireProcessInvitee = async (data) => {
    console.log(data);
    let response = await processHireInvitee(data, data.proposal_id);
    if (response == true) {
      Swal.fire({
        position: "center",
        icon: "success",
  ...
  const handleDeleteInvitee = (item) => {
    console.log(item);
  };
  ...
  return (
    <div className="tw-css">
          ...
          {activeTab === "Invites" && (
            <div>
              ...
              <ol className="relative">
                {invitedApplicants.map((item, index) => (
                  <div
                    className="w-full rounded-xl p-4 border flex items-start mb-4"
                    key={index}
                  >
                    ...
                    <div className="w-1/6 relative">
                      {item?.status == "accepted" && (
                        <button
                          className="border bg-green-600 text-white rounded-md px-4 py-2 
                       text-sm hover:bg-gray-200 mb-2 block"
                          onClick={() => handleInviteeInterviewSet(item)}
                        >
                          Interview
                        </button>
                      )}
                      ...
                      {item?.status == "interview" && (
                        <button
                          className="border bg-yellow-600 text-white rounded-md px-4 py-2 
                       text-sm hover:bg-gray-200 mb-2 block"
                          onClick={() => handleViewInviteeInterviewData(item)}
                        >
                          Interview Details
                        </button>
                      )}
                      ...
                      {openMenuTwo === index && (
                        <div className="absolute right-0 top-10 z-50 w-44 bg-white border rounded-md shadow-lg py-2">
                          <button
                            onClick={() => handleMenuTwoAction("view", item)}
                            className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 w-full text-left"
                          >
                            <Eye size={14} /> View Profile
                          </button>
                          {item?.status !== "pending" && (
                            <button
                              onClick={() => handleMenuTwoAction("hired", item)}
                              className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 w-full text-left"
                            >
                              <UserPlus size={14} /> Add To Talent Pool
                            </button>
                          ...
                          <button
                            onClick={() =>
                              handleMenuTwoAction("rejected", item)
                            }
                            className="flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-gray-100 w-full text-left"
                          >
                            <Trash2 size={14} /> Reject
                          ...
                          <button
                            onClick={() => handleMenuTwoAction("close")}
                            className="flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 w-full text-left"
                          >
                            <X size={14} /> Close
                          </button>
      ...
      />
      <InviteeInterviewModal
        isOpen={isInviteeInterviewOpen}
        onClose={() => setIsInviteeInterviewOpen(false)}
        candidateData={candidateData}
        status={interviewStatus}
      />
      {/* <MessageModal
      ...
      {isSalaryOpen && (
        <SalaryModal
          ...
          action={[handleHire, handleHireProcessInvitee]}
        />
      )}
    </div>
  );
