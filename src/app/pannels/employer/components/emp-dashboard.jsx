...
function EmpDashboardPage() {
	...
	}posted-jobs/count/${"user.id"}`;
    console.log("jobsCountUrl", jobsCountUrl);
    const notificationsUrl = `${process.env.REACT_APP_BASE_URL}notifications`;

	useEffect(() => {
		const postedJobsByEmployer = async () => {
			try {
                const [res1, res2] = await Promise.all([
                    axios.get(jobsCountUrl),
                    axios.get(notificationsUrl),
                ]) 
                console.log("res-count-jobs", res1, res2);
								const jobsData = res1;
                                const notificationsData = res2;
	...
	}, [jobsCountUrl, notificationsUrl]);

	return (
		<>
			<div className="">
				{showTopMessage && error && <p>Error getting data. Try again</p>}
			</div>
			<div className="">
